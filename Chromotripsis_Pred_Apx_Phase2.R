...
CNV=read.csv('/cbcb/project2-scratch/sanju/Chromotrypsis/2.Data/CN_State_of_Segments_CHAS_wdlocation.txt', sep='\t')

############
#Handling apx CNV
###########
CNV$CN.State=round(CNV$CN.State,0)
#CNV$CN.State=round(CNV$CN.State,0)
#CNV$CN.State=ceiling(CNV$CN.State)
...
############################################################################################################################################################################################
source('/home/sinhas8/Projects/Project_Chromotrypsis/3.Tools/Events_Enrichment_Test.R')

#*##Check this step again
ND_Sample=is.na(FilenameQC$TuScan.Ploidy)
ND_Tumour_Samples=FilenameQC$File[ND_Sample]

CNV=CNV[order(CNV$Full.Location),]
CNV_Type3=CNV[!is.na(match(CNV$File, ND_Tumour_Samples)),]
...
############################################################################################################################################################################################
source('/home/sinhas8/Projects/Project_Chromotrypsis/3.Tools/Oscillatory_Events_Enrichement_Test.R')
