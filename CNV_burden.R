...
temp_wdoutLOH=temp[temp$Type != 'LOH',]

temp_filtered=temp_wdoutLOH[,c(12, 2, 3, 4, 5, 7)]
...
###Combined both
#new_both=read.csv('/home/sinhas8/Downloads/From_OncoScan_COnsole/Segment_from_oncoscan_both_phases.csv')
both=read.table('/home/sinhas8/Downloads/From_OncoScan_COnsole/Both_Phases_combined.txt')
colnames(both)=c('FileName', 'Chromosome', 'StartPosition','StopPosition','Markers','State')
...
#	new_Loss_burden=sapply(both_byfileandchr, function(x) sum(sapply(x, function(y) 
 #                      sum(y$StopPosition[y$State< 2]-y$StartPosition[y$State< 2])))/(3.3*10^9) )
#	new_Gain_burden=sapply(both_byfileandchr, function(x) sum(sapply(x, function(y) 
 #                      sum(y$StopPosition[y$State> 2]-y$StartPosition[y$State> 2])))/(3.3*10^9) )
